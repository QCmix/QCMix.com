export default function BartendersPage() {
  return (
    <main className="min-h-screen bg-black text-white antialiased">
      <Header
        title="Bartenders"
        description="Career paths, longevity, and the people behind the bar."
      />
      <Feature
        title="The Ones Who Keep the Lights On"
        description="Experience and restraint matter more than volume."
      />
      <Grid title="Bartender Spotlights" />
    </main>
  )
}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="surface p-4">
              <div className="aspect-[4/3] qc-img" />
              <h3 className="mt-3 font-semibold">Editorial Feature</h3>
              <p className="mt-1 text-sm text-white/60">Profile & perspective.</p>
            </div>
          ))}
        </div>
      </div>
